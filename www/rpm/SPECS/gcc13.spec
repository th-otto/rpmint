...
%if "%{buildtype}" == "cross"
Provides:       %{crossmint}libm2 = %{version}-%{release}
Provides:       %{crossmint}gcc-m2 = %{version}-%{release}
%else
%if %{build_m2}
BuildRequires:  cross-mint-%{pkgname}-m2 = %{version}
%endif
Provides:       libm2 = %{version}-%{release}
...
%files m2
%defattr(-,root,root)
%{_rpmint_target_prefix}/bin/%{_rpmint_target}-gm2*
%{_rpmint_target_prefix}/%{_rpmint_target}/bin/gm2*
%if "%{buildtype}" != "cross"
%{_rpmint_target_prefix}/bin/gm2*
%endif
%{gccsubdir}/cc1gm2
%{gccsubdir}/m2
%{gccsubdir}/libm2*
...
%changelog
* Mon May 22 2023 Thorsten Otto <admin@tho-otto.de>
